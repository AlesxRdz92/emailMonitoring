...
//const circular = require('json-circular');
var files;
...
var mailbox = new ews.Mailbox('alejandro.gongora@wnco.com'); //It was possible to acces to the shared mailbox
let folderId = new ews.FolderId(ews.WellKnownFolderName.Inbox, mailbox);
console.log(folderId);
exch.SubscribeToStreamingNotifications([folderId], ews.EventType.NewMail, ews.EventType.FreeBusyChanged).then(streaming => {
    ...
    connection.OnDisconnect.push((connection, e) => {
        if (e.Exception)
            logging.log(3, 'Connection lost', JSON.stringify(e.Exception));
        else
            logging.log(1, 'Connection finished for the timeout');
        connection.Open();
        logging.log(1, 'Connection restarted');
    });
}).catch(e => {
